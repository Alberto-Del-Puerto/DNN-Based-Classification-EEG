...
def load_trained_model(model_path='reports/finally_model_audio100.keras'):
    """Carga un modelo previamente entrenado"""
    return tf.keras.models.load_model(model_path)

def evaluate_with_trained_model(psd_data, labels, 
                              model_path='reports/finally_model_audio100.keras',
                              scaler_path='reports/scaler_audio100.save',
                              threshold=0.5):
...
if __name__ == "__main__":
    psd_data = pd.read_csv('ProcessedData/Tetris_dataPSD_audio.csv', header=None).values
    msc_data = pd.read_csv('ProcessedData/Tetris_DataCoherence_audio.csv', header=None).values
    entropy_data = pd.read_csv('ProcessedData/Tetris_dataEntropy_audio.csv').values
